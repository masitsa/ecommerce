	...
	function makeSidebar() {
		...
		for (var i=0; i<gmarkers.length; i++) {
			if (gmarkers[i].getVisible()) {
				links += '<a href="javascript:myclick(' + i + ')">' + gmarkers[i].myname + '</a>&nbsp;&nbsp;<a href="edit.php?lat='+ gmarkers[i].position.lat() +'&lng='+ gmarkers[i].position.lng()+'">Edit</a>&nbsp;&nbsp;<a onclick="return confirm_delete()" href="delete_points.php?lat='+ gmarkers[i].position.lat() +'&lng='+ gmarkers[i].position.lng()+'">Delete</a><br/>';
			}
		}
	...
	function confirm_delete() {
		return confirm('Do you really want to delete this point?');
	}
	...
	function initialize() {
		...
		downloadUrl(CI.base_url+"admin/map/create_markers", function(doc) {
			...
			for (var i = 0; i < markers.length; i++) {
				...
				html.innerHTML = '<div><b>'+name+'</b>&nbsp;:&nbsp;<br/>'+info+'<br /></div>';
	...
	function toggle() {
		...
		if(ele.style.display == "none") {
			...
			text.innerHTML = "Hide Panel";
		} else {
			ele.style.display = "none";
			text.style.background = "#1FAEFF";
			text.style.color = "white";
			text.innerHTML = "Show Panel";
		}
	}